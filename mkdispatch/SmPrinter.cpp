...
void SmPrinter::InternalPrint(vector const& requests, Options const& options) {
	...
	for(size_t i = 0; i < stateNames.size(); ++i) {
		std::for_each(transitions.cbegin(), transitions.cend(), [&wasPrinted, &i, &stateNumbers](DfaState::map::value_type const& pair) {
			if(stateNumbers[pair.first] == i && wasPrinted.insert(i).second) {
				std::cout << "\t\t{";
				for(char ch = '\0'; ch >= 0; ++ch) {
					auto it = pair.second.find(ch);
					std::cout << (it == pair.second.cend() ? 0 : ch == '\n' ? -stateNumbers[it->second.Name] : stateNumbers[it->second.Name]) << ',';
				}
				std::cout << "}, // " << i << std::endl;
			}
		});
		if(wasPrinted.find(i) == wasPrinted.cend()) {
			std::for_each(transitions.cbegin(), transitions.cend(), [&wasPrinted, &i, &stateNumbers, &fnIndices](DfaState::map::value_type const& pair) {
				std::for_each(pair.second.cbegin(), pair.second.cend(), [&wasPrinted, &i, &stateNumbers, &fnIndices](std::pair<char, DfaState> const& pair) {
					if(stateNumbers[pair.second.Name] == i && wasPrinted.insert(i).second) {
						std::cout << "\t\t{";
